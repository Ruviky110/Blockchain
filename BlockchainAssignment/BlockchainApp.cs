...
namespace BlockchainAssignment
{
    public partial class BlockchainApp : Form
    {
        ...
        {
            Transaction transaction = new Transaction(publicKey.Text, reciever.Text, Double.Parse(amount.Text), Double.Parse(fee.Text), "InvalidPrivateKey");
            blockchain.transactionPool.Add(transaction);
            UpdateText(transaction.ToString());
            ...
            if (!isValid)
            {
                UpdateText("Transaction signature is not valid.");
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            double totalAmount = Double.Parse(amount.Text) + Double.Parse(fee.Text);

            if (balance < totalAmount)
            {
                UpdateText("Insufficient balance to complete the transaction.");
                return;
            }
